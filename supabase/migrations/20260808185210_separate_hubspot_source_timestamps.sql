-- Keep the two source timestamps distinct. last_deal_at remains the efficient
-- sort key and is the last-modified value with created-at as its fallback.
alter table public.companies
  add column if not exists last_hubspot_created_at timestamptz,
  add column if not exists last_hubspot_modified_at timestamptz;

update public.companies c
set
  last_hubspot_created_at = source.last_hubspot_created_at,
  last_hubspot_modified_at = source.last_hubspot_modified_at,
  last_deal_at = source.last_hubspot_modified_at
from (
  select
    d.owner_id,
    d.company_id,
    max(d.hubspot_created_at) as last_hubspot_created_at,
    max(coalesce(d.hubspot_modified_at, d.hubspot_created_at)) as last_hubspot_modified_at
  from public.deals d
  where d.company_id is not null
  group by d.owner_id, d.company_id
) source
where c.id = source.company_id and c.owner_id = source.owner_id;

update public.companies c
set
  last_hubspot_created_at = null,
  last_hubspot_modified_at = null,
  last_deal_at = null
where not exists (
  select 1
  from public.deals d
...
begin
  ...
  if affected_company is not null then
    update public.companies c
    set
      last_hubspot_created_at = (select max(d.hubspot_created_at) from public.deals d where d.owner_id = affected_owner and d.company_id = affected_company),
      last_hubspot_modified_at = (select max(coalesce(d.hubspot_modified_at, d.hubspot_created_at)) from public.deals d where d.owner_id = affected_owner and d.company_id = affected_company),
      last_deal_at = (select max(coalesce(d.hubspot_modified_at, d.hubspot_created_at)) from public.deals d where d.owner_id = affected_owner and d.company_id = affected_company)
  ...
  if tg_op = 'UPDATE' and old.company_id is distinct from new.company_id and old.company_id is not null then
    update public.companies c
    set
      last_hubspot_created_at = (select max(d.hubspot_created_at) from public.deals d where d.owner_id = old.owner_id and d.company_id = old.company_id),
      last_hubspot_modified_at = (select max(coalesce(d.hubspot_modified_at, d.hubspot_created_at)) from public.deals d where d.owner_id = old.owner_id and d.company_id = old.company_id),
      last_deal_at = (select max(coalesce(d.hubspot_modified_at, d.hubspot_created_at)) from public.deals d where d.owner_id = old.owner_id and d.company_id = old.company_id)
    where c.id = old.company_id and c.owner_id = old.owner_id;
  end if;
  return null;
end $$;

drop function if exists public.company_dashboard_page(text, text, text, boolean, boolean, date, date, integer, integer);
